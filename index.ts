...
const MEMBER_EXPR = "[a-zA-Z_$][\\w$]*(?:(?:\\.|\\?\\.)[a-zA-Z_$][\\w$]*)+";
...
function mapAttrNameInList(attrs: string): string {
  ...
  return s.replace(/(\s+)(@?[\w.:-]+)=/g, (full: string, ws: string, rawName: string) => {
...
function transformSvgTagChunk(chunk: string): string {
  ...
  const newName = RNSVG_TAG_SET.has(rawName) ? pascalComponentToSvgTag(rawName) : rawName;
...
function coercePropsBraceExpressions(fragment: string): string {
  ...
  for (let i = 0; i < 24 && s !== prev; i++) {
    ...
    s = s.replace(new RegExp(`=\\{\\s*${M}\\s*\\?\\?\\s*'([^']*)'\\s*\\}`, "g"), '="$1"');
    s = s.replace(new RegExp(`=\\{\\s*${M}\\s*\\?\\?\\s*"([^"]*)"\\s*\\}`, "g"), '="$1"');
    s = s.replace(
      new RegExp(`=\\{\\s*${M}\\s*\\|\\|\\s*(-?\\d+(?:\\.\\d+)?)\\s*\\}`, "g"),
      '="$1"',
    ...
    s = s.replace(new RegExp(`=\\{\\s*${M}\\s*\\|\\|\\s*"([^"]*)"\\s*\\}`, "g"), '="$1"');
    s = s.replace(
      new RegExp(`=\\{\\s*${M}\\s*\\?\\s*${M}\\s*:\\s*(-?\\d+(?:\\.\\d+)?)\\s*\\}`, "g"),
      '="$1"',
    );
    s = s.replace(new RegExp(`=\\{\\s*${M}\\s*\\?\\s*${M}\\s*:\\s*'([^']*)'\\s*\\}`, "g"), '="$1"');
    s = s.replace(new RegExp(`=\\{\\s*${M}\\s*\\?\\s*${M}\\s*:\\s*"([^"]*)"\\s*\\}`, "g"), '="$1"');
  }
  s = s.replace(new RegExp(`\\s+([a-zA-Z][\\w.:-]*)=\\{\\s*${M}\\s*\\}`, "g"), "");
  return s;
}
...
function splitTopLevelObjectEntries(objLiteral: string): string[] {
  ...
  for (let i = 0; i < body.length; i++) {
    ...
    if (c === "," && depthBraces === 0 && depthBrackets === 0 && depthParens === 0) {
...
function parseTopLevelKeyValue(entry: string): { key: string; value: string } | null {
  let inString: '"' | "'" | null = null;
  let escape = false;
  let depthBraces = 0;
  let depthBrackets = 0;
...
function resolveImportTargetPath(sourceFilePath: string, importPath: string): string | null {
...
type EvalState = { kind: "known"; value: StaticLiteral } | { kind: "unknown" };
...
function extractJsxExpressionBody(
  s: string,
  eqIdx: number,
): {
  start: number;
...
function parseSourceSymbols(sourceFilePath: string, sourceText: string): SourceSymbols {
  const sf = ts.createSourceFile(
    sourceFilePath,
    sourceText,
    ts.ScriptTarget.Latest,
...
function binaryNumberOp(op: ts.SyntaxKind, l: number, r: number): StaticLiteral | undefined {
  switch (op) {
...
function buildExpressionLiteralResolver(
  ...
  function resolveConst(
    ...
    if (imported) {
      const resolved = resolveConst(imported.sourcePath, imported.importedName, false, seen);
  ...
  function resolveIdentifier(filePath: string, name: string, seen: Set<string>): EvalState {
...
function regionStartAfterNewline(tsxContent: string, matchIndex: number): number {
...
function extractAllSvgComponents(tsxContent: string): SvgComponentExport[] {
  ...
  for (let i = 0; i < regions.length; i++) {
    const end = i + 1 < regions.length ? regions[i + 1]?.start : tsxContent.length;
...
function svgContainsNanoUnsupportedElement(svg: string): boolean {
  return (
    svgContainsFilterOrMask(svg) || /<\s*(?:foreignObject|image|use|text|textPath)\b/i.test(svg)
  );
}
...
function convertTSXToSVG(inputFilePath: string): { outputPath: string; svgContent: string } | null {
  try {
...
function convertTSXToSvgFolder(
  ...
  try {
    ...
    if (components.length === 0) {
      console.error("Error: No exported components with <Svg> found in the input file");
      return null;
    }
    ...
    for (const { name, svgFragment } of components) {
      ...
      if (nano && svgContainsNanoUnsupportedElement(transformed)) {
        console.error(`Skipped (nano: unsupported SVG element): ${name}`);
...
async function processFileBatch(
  ...
  for (let i = 0; i < inputs.length; i += concurrency) {
    ...
    for (const result of results) {
      ...
      console.log(
        `${dryPrefix}Wrote ${wroteN} file(s)${nano && skipN > 0 ? `, skipped ${skipN} (nano: unsupported SVG elements)` : ""}${nonConvertibleN > 0 ? `, skipped ${nonConvertibleN} non-convertible` : ""} to ${batch.outputDir}:`,
      );
      for (const f of batch.files) {
        console.log(`  ${dryPrefix}${path.basename(f.outputPath)} (${f.componentName})`);
      }
    }
  }
  ...
  if (nonConvertible.length > 0) {
    ...
    for (const item of nonConvertible) {
      const source = multi ? ` (${path.relative(process.cwd(), item.inputFilePath)})` : "";
...
async function runConversion(
  ...
  const defaultRoot = path.resolve(process.cwd(), opts.input ?? "src/components/svgs");
  ...
  if (inputArg === undefined || inputArg === "") {
    ...
    console.log(`Scanning ${defaultRoot}: ${inputs.length} file(s) with <Svg>`);
  } else if (fs.existsSync(inputArg) && fs.statSync(inputArg).isDirectory()) {
    inputs = discoverSvgSourceFiles(inputArg);
    if (inputs.length === 0) {
      console.error(`Error: No <Svg> sources found under ${path.resolve(inputArg)}`);
      return 1;
    }
    console.log(`Scanning ${path.resolve(inputArg)}: ${inputs.length} file(s)`);
  } else {
    const ext = path.extname(inputArg).toLowerCase();
    if (ext !== ".tsx" && ext !== ".ts") {
      console.error("Error: Input must be a .tsx / .ts file or a directory");
      return 1;
    }
...
function main() {
  ...
  program
    .name("svg-convert")
    .description("Convert react-native-svg TSX to static .svg (string transforms).")
    ...
    .argument("[input]", "Input .tsx/.ts file or directory; omit to scan --input")
    .argument("[outputDir]", "Output directory; default is the same input file folder")
    .option(
      "-i, --input <dir>",
    ...
    .option("--nano", "Skip filter/mask elements or mask=/filter= url attrs (font/nano)")
    .option("--concurrency <number>", "Max parallel file conversions (default: 4)", "4")
    .option("--dry-run", "Preview output without writing files")
    .option("--watch", "Re-run conversion on file changes (concurrency forced to 1)")
    .action(async (inputArg: string | undefined, outputDirArg: string | undefined) => {
      ...
      const concurrency = watch
        ? 1
        : Math.min(16, Math.max(1, parseInt(opts.concurrency ?? "4", 10) || 4));
      ...
      if (watch) {
        const watchDir =
          inputArg && fs.existsSync(inputArg) && fs.statSync(inputArg).isDirectory()
  ...
  const previewCommand = program
    .command("font-preview [fonts...]")
    .description("Generate a standalone HTML preview for local font files (TTF/OTF/WOFF).")
    .option("-o, --output <file>", "HTML output path (default: preview.html beside the first font)")
    .action(async (fonts: string[] = []) => {
      const options = previewCommand.opts<{ output?: string }>();
      if (!fonts || fonts.length === 0) {
        console.error("Error: specify one or more font file paths");
        process.exit(1);
      }
      const fontsData = fonts.map((fp) => ({
        path: path.resolve(fp),
        name: path.basename(fp),
        buffer: Buffer.alloc(0),
      }));
      for (const f of fontsData) {
        if (!fs.existsSync(f.path) || !fs.statSync(f.path).isFile()) {
          console.error(`Font not found: ${f.path}`);
          process.exit(1);
        }
        f.buffer = fs.readFileSync(f.path);
      }
      // Parse fonts to collect glyph unicodes (best-effort)
      const fontInfos = fontsData.map((f) => {
        ...
        try {
          ...
          try {
            ...
            throw new Error(
              `Unable to parse font "${f.path}": ${error instanceof Error ? error.message : String(error)}`,
            );
          }
        }
        const glyphs: { unicode: number; name?: string }[] = [];
        if (font?.glyphs && typeof font.glyphs.get === "function") {
          for (let i = 0; i < font.glyphs.length; i++) {
            const glyph = font.glyphs.get(i);
            if (glyph?.unicode) {
              glyphs.push({ unicode: glyph.unicode, name: glyph.name });
            }
          }
        }
        const glyphmapPath = path.join(
          path.dirname(f.path),
          `${path.basename(f.path, path.extname(f.path))}.glyphmap.json`,
        );
        const glyphmapGlyphs = new Map<number, string>();
        if (fs.existsSync(glyphmapPath)) {
          try {
            const glyphmap = JSON.parse(fs.readFileSync(glyphmapPath, "utf8"));
            for (const [iconName, entry] of Object.entries(glyphmap.i ?? {})) {
              for (const layer of (entry as { 1?: [number, string][] })[1] ?? []) {
                if (typeof layer[0] === "number") {
                  glyphmapGlyphs.set(layer[0], iconName);
                }
              }
            }
          } catch (error) {
            console.error(`Warning: unable to read glyphmap ${glyphmapPath}:`, error);
          }
        }
        return {
          ...f,
          fontFamily: `font-preview-${fontsData.indexOf(f)}`,
          glyphs: glyphs.map((glyph) => ({
            ...glyph,
            name:
              glyphmapGlyphs.get(glyph.unicode) ??
              glyph.name ??
...
<style>
 :root { color-scheme: light dark; --background: #fff; --foreground: #111; --muted: #666; --border: #eee; --surface: #fff; --surface-hover: #f7f7f7; }
@media (prefers-color-scheme: dark) {
 :root { --background: #181818; --foreground: #f5f5f5; --muted: #aaa; --border: #444; --surface: #222; --surface-hover: #2d2d2d; }
}
body { font-family: system-ui, -apple-system, "Segoe UI", Roboto, Helvetica, Arial; padding: 16px; background: var(--background); color: var(--foreground); }
.font-block { margin-bottom: 40px; }
.sample { font-family: inherit; font-size: 28px; display:inline-flex; flex-direction:column; justify-content:center; width:7rem; min-height:5rem; text-align:center; border:1px solid var(--border); margin:2px; padding:6px; line-height:2.5rem; box-sizing:border-box; cursor:pointer; background:var(--surface); color:var(--foreground); }
.sample:hover { border-color:var(--muted); background:var(--surface-hover); }
.icon-name { display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font:12px system-ui, sans-serif; line-height:1.2; color:var(--muted); }
.copied { color:#35b86f; }
.grid { display:flex; flex-wrap:wrap; max-width: 100%; }
...
${fontInfos
  .map((fi: any) => {
    const mime = mimeForExt(path.extname(fi.path));
    const fontFamily = escapeHtml(fi.fontFamily);
    const fontName = escapeHtml(fi.name);
    return `<style>@font-face{font-family:'${fontFamily}';src:url('data:${mime};base64,${fi.data}') format('${formatForExt(path.extname(fi.path))}');font-weight:normal;font-style:normal;}</style><div class="font-block"><div class="header"><strong>${fontName}</strong> <small style="color:#666">(${fi.glyphs.length} glyphs)</small></div><div class="grid" style="font-family:'${fontFamily}';">${fi.glyphs
      .slice(0, 1000)
      .map((g: any) => {
        const name = escapeHtml(g.name ?? "");
        return `<button class="sample" style="font-family:'${fontFamily}';" type="button" data-icon-name="${name}" title="Copy ${name}"><span>${String.fromCodePoint(g.unicode)}</span><span class="icon-name">${name}</span></button>`;
      })
      .join("")}</div></div>`;
  })
  .join("\n")}
<script>
document.querySelectorAll('[data-icon-name]').forEach((icon) => {
	icon.addEventListener('click', async () => {
		const name = icon.dataset.iconName;
		if (!name) return;
		try {
			await navigator.clipboard.writeText(name);
			const label = icon.querySelector('.icon-name');
			if (label) {
			...
			window.prompt('Copy icon name:', name);
		}
	});
});
</script>
      ...
      const outputPath = path.resolve(
        options.output ?? path.join(path.dirname(fontsData[0].path), "preview.html"),
      );
      fs.writeFileSync(outputPath, html, "utf8");
      console.log(`Font preview written to ${outputPath}`);
      if (process.platform === "darwin") {
        const opener = Bun.spawn(["open", outputPath], {
          stdout: "ignore",
          stderr: "pipe",
        });
        const exitCode = await opener.exited;
        if (exitCode !== 0) {
          const stderr = await new Response(opener.stderr).text();
          throw new Error(
            `Unable to open font preview: ${stderr.trim() || `open exited with code ${exitCode}`}`,
          );
        }
      } else {
        console.log(`Open this file in a browser: ${outputPath}`);
      }
    });
